...
const HowItWorks = () => {
  return (
    <div className=" w-full mt-15 px-3 md:pl-10 py-5 h-[60vh]">
      <h1 className="text-[var(--accent)] text-center text-[27px] lg:text-[40px] md:text-[35px] font-[900]">Get Started With EDUSCRIBE</h1>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3 md:gap-2 mt-5 lg:mt-10">
       {data.map((item, index) => (
        ...
        <div key={index} className="flex flex-row gap-2 h-[60px] md:flex-col lg:mx-auto">
          ...
          <div className="bg-[var(--accent)] w-5 h-5 md:w-10 md:h-10 rounded-full 
          ...
          <div className="md:flex md:flex-col md:gap-2 md:items-center md:justify-center">
            <h1 className="text-[15px] md:text-[18px] lg:text-[20px] font-[700] text-[var(--accent)] md:text-center">{item.title}</h1>
            <p className="text-[11px] md:text-center md:text-[13px] md:w-[210px] lg:w-[250px] lg:mt-1 font-[700] text-[var(--accent)]">{item.description}</p>                
          </div>
        </div>
       ))}
      </div>
